...
/**
 * Used to create, update or delete project variable definitions in the API.
 * Will automatically invalidate the project variable definitions and force get query to be executed again.
 */
export function useSaveVariablesMutation(projectName: string, previousVariables: VariableResource[]) {
  const queryClient = useQueryClient();

  return useMutation<VariableResource[], Error, VariableResource[]>({
    mutationKey: [resource, projectName, variableResource],
    mutationFn: (variables: VariableResource[]) => {
      const result: Array<Promise<any>> = [];

      for (const variable of variables) {
        if (previousVariables.find((prevVariable) => prevVariable.metadata.name === variable.metadata.name)) {
          // update
          result.push(updateVariable(variable));
        }
        // create
        result.push(createVariable(variable));
      }

      for (const prevVariable of previousVariables) {
        if (!variables.find((variable) => variable.metadata.name === prevVariable.metadata.name)) {
          // delete
          result.push(deleteVariable(prevVariable));
        }
      }
...
export function useUpdateVariablesMutation(projectName: string) {
  const queryClient = useQueryClient();
...
export function useDeleteVariablesMutation(projectName: string) {
  const queryClient = useQueryClient();
  ...
  return useMutation<VariableResource, Error, VariableResource>({
    ...
    onSuccess: (variable) => {
      queryClient.removeQueries([resource, variable.metadata.project, variable.metadata.name]);
